  ...
  public function checkout() {
    ...
    if ($this->currentUser()->isAnonymous() && !$cart_config->get('checkout_anonymous')) {
      ...
      if ($this->config('user.settings')->get('register') != USER_REGISTER_ADMINISTRATORS_ONLY) {
        drupal_set_message($this->t('If you do not have an account yet, you should <a href="http://ecomstaging.leespring.com/user/login"></a>register now.', [':url' => Url::fromRoute('user.register', [], ['query' => drupal_get_destination()])->toString()]));
      }
      return $this->redirect('user.login', [], ['query' => drupal_get_destination()]);
    }
